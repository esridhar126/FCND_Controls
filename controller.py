...
class NonlinearController(object):
    ...
    def altitude_control(self, altitude_cmd,
        ...
        i_cmd = self.Kp_alt * (altitude_cmd - altitude) + vertical_velocity_cmd

        # Limit the ascent/descent rate
        if i_cmd > self.max_ascent_rate:
            i_cmd = self.max_ascent_rate
        elif i_cmd < -self.max_descent_rate:
            i_cmd = -self.max_descent_rate

        acceleration_cmd = DRONE_MASS_KG * acceleration_ff + \
                           self.Kp_i * (i_cmd - vertical_velocity)


        # cos(roll) * cos(pitch) of the attitude
        rot_mat_33 = euler2RM(attitude[0], attitude[1], attitude[2])
        b_z = rot_mat_33[2, 2]
